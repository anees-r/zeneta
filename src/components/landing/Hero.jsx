...
import LiveStreamEmbed from "@/components/landing/LiveStreamEmbed";
    ...
    async function load() {
      try {
        ...
        for (const g of games) {
          const overlay = await fetch(`/api/overlay/${g.slug}`).then((r) =>
            r.json(),
          );
...
function StatCards() {
  ...
  const isLive = Boolean(liveSettings?.isLive);
  return (
    <div className={`${isLive ? 'flex flex-col' : 'grid grid-cols-2'} gap-3`}>
      <div className="p-4 rounded-xl border border-violet-500/15 bg-violet-950/20">
        <div className="text-xs text-white/35 uppercase tracking-widest mb-2">
          Main Game
        </div>
...
function LiveOverlayPreview({ overlay, game }) {
  return (
    <div className="p-5 rounded-xl border border-violet-500/20 bg-violet-950/20">
      ...
      <div className="flex items-baseline gap-4 mb-4">
        ...
        {!overlay && (
          <span className="text-xs text-white/20 ml-auto">preview</span>
        )}
      </div>
    ...
    <section className="relative px-6 md:px-12 pt-20 pb-16">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center max-w-8xl">
        <div>
          ...
          <h1
            className="text-7xl md:text-9xl text-white tracking-[0.15em] uppercase leading-none mb-6"
          ...
          <p className="text-sm text-white/25 tracking-wide max-w-md mb-10">
            Pakistan-based gamer & creator. Valorant, CS2, story-mode games,
            anime & books. Probably hard stuck but having fun.
          </p>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <a
              href="https://instagram.com/zenetagram"
              target="_blank"
              rel="noopener noreferrer"
        ...
        {isLive ? (
          <div className="grid grid-cols-1 gap-3 xl:grid-cols-[minmax(0,0.95fr)_minmax(280px,1.3fr)]">
            <div className="flex flex-col gap-3">
              <StatCards />
              <LiveOverlayPreview overlay={overlay} game={game} />
            </div>
            <LiveStreamEmbed settings={liveSettings} />
          </div>
        ) : (
