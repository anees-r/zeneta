...
export default function StudioPage() {
  ...
  return (
    <main className="min-h-screen" style={{ background: "#080b08" }}>
      {/* Top bar */}
      <header className="border-b border-lime-400/[0.07] px-6 md:px-10 py-4 flex items-center justify-between sticky top-0 z-20"
        style={{ background: "#080b08" }}>
        <div className="flex items-center gap-4">
          <Link href="/" className="flex items-baseline gap-0.5">
            <span className="text-lg text-white tracking-[0.2em] uppercase"
              style={{ fontFamily: "DarkestSaturday, serif" }}>ZENETA</span>
            <span className="text-lime-400 text-xs">.gg</span>
          </Link>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button variant="ghost" size="sm" onClick={() => signOut({ callbackUrl: "/" })}
            className="gap-1.5 text-white/35 hover:text-white text-xs">
        ...
        <Tabs defaultValue="sessions">
          <TabsList>
            <TabsTrigger value="sessions" className="gap-2">
              <Radio className="w-3.5 h-3.5" /> Sessions
            </TabsTrigger>
            <TabsTrigger value="games" className="gap-2">
              <Gamepad2 className="w-3.5 h-3.5" /> Games
            </TabsTrigger>
            <TabsTrigger value="peak-ranks" className="gap-2">
              <Trophy className="w-3.5 h-3.5" /> Peak Ranks
            </TabsTrigger>
          </TabsList>
